...
public class ItemTooltipUI : MonoBehaviour
{
    ...
    /// </summary>
    public void InitItemUI(ItemDef item)
    {
        ...
        sb.AppendFormat("<size={0}><color={1}>{2}</color></size>\n",
                        titleSize, titleColor, item.DisplayName);

        // Description
        sb.AppendFormat("<size={0}><color={1}>{2}</color></size>\n\n",
                        descriptionSize, descriptionColor, item.Description);

        // Modifiers (one per line)
        foreach (var line in item.ModifiersDescription.Split('\n'))
        {
